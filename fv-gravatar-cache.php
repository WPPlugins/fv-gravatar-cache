...
Class FV_Gravatar_Cache {
  ...
  */
  function Cron( $email, $size = '96' ) {
    ...
    if( $not_in_cache ) {
      $this->Log( 'INSERT, '.$gravatar_downloaded .', '.$size.', '.$email.', '.date(DATE_RFC822)."\r\n" );
  ...
  function GetAvatar( $image, $id_or_email ) {
    ...
    if( !isset( $gravatars[$comment->comment_author_email] ) ) {
      $not_in_cache = true;;
    }
  ...
  function OptionsHead() {
      if(stripos($_SERVER['REQUEST_URI'],'/options-general.php?page=fv-gravatar-cache')!==FALSE) {
          ...
          if(isset($_POST['fv_gravatar_cache_save'])) {
              ...
              $this->Cache( 'default', '', $options['size'] );
      ...
      <form id="gravatarcache" action="" method="post">
        <table class="form-table">
          <tbody>
            ...
            <tr valigin="top">
              <th scope="row">Default Gravatar:<br /><small>(you need to visit this settings screen for update after you change 'Default Avatar' in Wordpress 'Discussion Settings')</small></th><td><img src="<?php echo $this->Cache( 'default' ); ?>" /></td>
            </tr>
            <tr valigin="top">
            ...
            <tr valigin="top">
              <th scope="row">Debug mode:</th><td><input name="debug" type="checkbox" <?php if( $options['debug'] == true ) echo 'checked="yes" '; ?> /> <small>(check <a href="<?php echo $this->GetCacheURL().'log.txt'; ?>">log.txt</a> file in Cache directory)</small></td>
            </tr>
...
function fv_gravatar_cache_activation() {
    global $wpdb;
		...
		wp_schedule_event( time()+3600, 'hourly', 'fv_gravatar_cache_cron' );
